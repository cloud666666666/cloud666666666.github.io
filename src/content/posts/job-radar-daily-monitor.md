...
**龙哥表(腾讯文档 smartsheet)**:一个每日更新的 27 届秋招汇总表,百余家公司(动态增长,当前 170+),含内推码、截止时间。有意思的是它前端是 canvas 渲染、DOM 里抓不到数据——最后从它的网络请求里挖到了公开 JSON 接口(`/dop-api/get/sheet`),数据是 base64 + zlib 压缩的,解开就是完整表格。
